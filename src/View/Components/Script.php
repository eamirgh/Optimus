...
class Script extends Component
{
    ...
    public function render(): View|string
    {
        ...
        if ($this->strategy === 'beforeInteractive') {
            ...
            return sprintf('<script%s>{{ $slot }}</script>', $idAttr);
        }
        ...
        if ($this->strategy === 'lazyOnload') {
            ...
            return sprintf('<script%s>window.addEventListener(\'load\',function(){{!! $slot !!}});</script>', $idAttr);
        }
        ...
        return sprintf('<script%s>document.addEventListener("DOMContentLoaded",function(){{!! $slot !!}});</script>', $idAttr);
    }
}
